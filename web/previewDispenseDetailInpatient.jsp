    ...
    <body>
        <form action="Controller?process=inDispenseProcess&action=add" method="post" onsubmit="return validateMedicine()">
            <table  width="100%">
                <thead>
                    <tr  >
                        <td colspan="3" class="headerText"><img align="left" src="images/back1.png" style="cursor: pointer" title="click to close" onclick="backButtton1()" />Pharmacy In-Patient Details</td>
                    </tr>
                    <tr  >
                        <td colspan="3" class="headerText">In-Patient Details</td>
                    </tr>

                </thead>
                <tbody>
                    <c:forEach end="1" items="${dispenseInpatient.indispensedmedicinebills}" var="indispensedmedicinebill">
                    <tr><td style="width: 45%">Invoice No: <c:out value="${indispensedmedicinebill.id}"/></td></tr>
                      </c:forEach>
                    <tr>
                        <td style="width: 45%">Patient Name: <c:out value="${dispenseInpatient.inpatient.name}" /></td>
                        <td style="width: 20%">Contact No:  <c:out value="${dispenseInpatient.inpatient.telephoneno}" /></td>
                        <td>Dispense Date: <fmt:formatDate pattern="dd MMMM yyyy " value="${dispenseInpatient.dispensedate}" /> </td>

                    </tr>

                    <c:forEach end="1" items="${dispenseInpatient.indispensedmedicinebills}" var="indispensedmedicinebill">
                            <tr>
                        <td style="width: 45%">Card No: <c:out value="${dispenseInpatient.inpatient.cardno}"/></td>
                        <td style="width: 30%">Doctor: <c:out value="${indispensedmedicinebill.doctorname}" /></td><!-- dispense.dispensedmedicinebills.doctordepartment -->
                        <td >Department :  <c:out value="${indispensedmedicinebill.doctordepartment}" /></td>

                    </tr>
                              <tr>
              <td colspan="3" align="right" style="padding-right:150px;display: none;">MISC Amount : <c:out value="${indispensedmedicinebill.miscellanousamount}" /></td>
              </tr>
                    </c:forEach>
            ...
            <TABLE id="dataTable" width="100%" border="1" >
                ...
                <tr>
                    <td align="center" >
                       <a id="print" href="Controller?process=inDispenseProcess&action=printBill&id=<c:out value="${dispenseInpatient.id}"/>">Print</a>
